...
class CheckpointsCallback(Callback):
    ...
    def on_epoch_end(
        ...
        """Save a resumable checkpoint after the epoch scheduler step."""
        if not self.save_each_epoch:
            return

        logger.info("Saving epoch %d training state...", global_epoch)
        ...
        if len(self.keep) > self.max_to_keep:
            ...
            if os.path.isfile(ckpt_to_delete):
                logger.info("Deleting old epoch checkpoint %s", ckpt_to_delete)
    ...
    def __call__(
        ...
        """
        if not isValPhase:
            if self.save_periodic and global_step % self.save_freq == 0:
            ...
            if k not in logs:
                raise KeyError(
                    f"Checkpoint metric '{k}' is absent from validation logs: "
                    f"{sorted(logs.keys())}"
                )

            v = float(logs[k])
            lower_is_better = k.startswith("loss")
            previous = self.best_val.get(k)
            improved = (
                previous is None
                or (lower_is_better and v < previous)
                or (not lower_is_better and v > previous)
            )

            if improved:
                logger.info(
                    "Model %s improved from %s to %.6f, saving checkpoint...",
                    k,
                    "unset" if previous is None else f"{previous:.6f}",
                    v,
                )
                self.best_val[k] = v
                self.epochs_without_improvement = 0
                ...
                if self.save_all_states:
                    self.best_path = trainer.save_all_states(
                        best_dir,
                        0,
                        0,
                    )
                else:
                    self.best_path = trainer.save_weights(best_dir, 0)
            else:
            ...
            if (
                ...
                logger.info(
                    "Early stopping triggered: %s did not improve for %d "
                    "consecutive epochs.",
                    k,
                    self.early_stopping_patience,
                )
