...
class TorchTrainer(ABC, nn.Module):
    ...
    def train_epoch(
        ...
        if eval_data is not None:
            ...
            for batch in tqdm.tqdm(eval_data, ascii=True):
                ...
                if "preds" in val_log and "labels" in val_log:
                    all_preds.extend(val_log.pop("preds"))
                    all_labels.extend(val_log.pop("labels"))
            ...
            ua_acc_val = None
            macro_f1_val = None
            weighted_f1_val = None

            if len(all_preds) > 0 and len(all_labels) > 0:
                ua_acc_val = recall_score(
                    all_labels,
                    all_preds,
                    average="macro",
                    zero_division=0,
            ...
            for key, value in eval_logs.items():
                ...
                postfix += f"{key}: {mean_val:.4f} "
                mlflow.log_metric(f"val_{key}", mean_val, step=step)

            if ua_acc_val is not None:
                postfix += f"| UA-Acc: {ua_acc_val:.4f} "
                mlflow.log_metric("val_ua_acc", ua_acc_val, step=step)

            if macro_f1_val is not None:
                postfix += f"| Macro-F1: {macro_f1_val:.4f} "
                mlflow.log_metric("val_macro_f1", macro_f1_val, step=step)

            if weighted_f1_val is not None:
                postfix += f"| Weighted-F1: {weighted_f1_val:.4f} "
                mlflow.log_metric("val_weighted_f1", weighted_f1_val, step=step)
            ...
            if callbacks is not None:
                ...
                if ua_acc_val is not None:
                    eval_logs["ua_acc"] = ua_acc_val
                if macro_f1_val is not None:
                    eval_logs["macro_f1"] = macro_f1_val
                if weighted_f1_val is not None:
                    eval_logs["weighted_f1"] = weighted_f1_val
    ...
    def evaluate(
        ...
        for batch in tqdm.tqdm(test_data, ascii=True):
            ...
            if "preds" in test_log and "labels" in test_log:
                all_preds.extend(test_log.pop("preds"))
                all_labels.extend(test_log.pop("labels"))

            for key, value in test_log.items():
                test_logs.setdefault(key, []).append(value)
        ...
        for key, value in test_logs.items():
            mean_val = np.mean(value)
            final_logs[key] = mean_val
            postfix += f"{key}: {mean_val:.4f} "

            try:
                mlflow.log_metric(f"test_{key}", mean_val, step=self.global_step)
            except:
    ...
    def load_all_states(self, path: str, device: str = "cpu"):
        ...
        if hasattr(self, "scaler") and "scaler" in dict_checkpoint:
            self.scaler.load_state_dict(dict_checkpoint["scaler"])
        ...
        with mlflow.start_run():
            for epoch in range(self.start_epoch, epochs + 1):
                ...
                if getattr(self, "should_stop", False):
                    logger.info(
                        "Stopping training after epoch %d because the "
                        "validation metric reached the configured patience.",
                        epoch,
                    )
                    break
